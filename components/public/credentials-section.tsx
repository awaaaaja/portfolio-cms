...
function CredentialGroup({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="min-w-0 border-y border-white/[0.07] bg-white/[0.015] py-7">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <HorizontalAutoSlider ariaLabel={`${title} carousel`} intervalMs={4000} itemClassName="flex w-[min(86vw,30rem)]">
          {children}
...
function CredentialCard({ title, meta, description, href, image, label = "View credential" }: { title: string; meta: string; description?: string | null; href?: string | null; image?: string | null; label?: string }) {
  ...
  const content = (
    <article className="interactive-card glass group flex h-[27rem] w-full flex-col overflow-hidden rounded-xl transition duration-500 hover:border-cyan-200/25 sm:h-[17rem] sm:flex-row">
      <div className="flex h-48 w-full shrink-0 items-center justify-center overflow-hidden bg-white p-2 sm:h-full sm:w-48">
      ...
      <div className="flex min-w-0 flex-1 flex-col p-4 sm:p-5">
        ...
        <h4 className="mt-2 line-clamp-2 text-lg font-bold text-white">{title}</h4>
