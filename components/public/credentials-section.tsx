      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-2">
        ...
        <CredentialGroup title="Publications" icon={<BookOpenText className="h-5 w-5" />}>
          {publications.map((item) => (
            <CredentialCard key={item.id} title={item.title} meta={`${item.publisher || "Publication"} / ${formatDate(item.published_at)}`} description={item.description} href={item.publication_url} image={item.cover_url} />
          ))}
        </CredentialGroup>
      </div>
    </section>
...
function CredentialCard({ title, meta, description, href, image }: { title: string; meta: string; description?: string | null; href?: string | null; image?: string | null }) {
  return (
    <Reveal>
      <article className="interactive-card glass group flex min-h-40 flex-col overflow-hidden rounded-xl min-[420px]:flex-row">
        ...
        <div className="flex min-w-0 flex-1 flex-col p-4 sm:p-5">
          ...
          {href ? <Link href={href} className="mt-auto inline-flex items-center gap-1 pt-4 text-xs font-medium text-cyan-200" data-cursor="hover">View credential <ExternalLink className="h-3.5 w-3.5" /></Link> : null}
        </div>
      </article>
    </Reveal>
  );
